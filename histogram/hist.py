pelis=rated_movies.where(rated_movies["title"]=='Forrest Gump (1994)').dropna()

pelis["rating"]=pd.to_numeric(pelis["rating"])
plt.hist(pelis['rating'])
count=rated_movies.groupby('title')["title"].count().reset_index(name="count")
count_sort=count.sort('count', ascending=False)
top10=count_sort[0:20]
plt.bar(range(len(top10)), top10['count'])
plt.xticks(range(len(top10)), top10['title'], rotation=45, size=10)
